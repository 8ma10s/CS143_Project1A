...
CREATE TABLE Sales(mid INT, ticketsSold INT, totalIncome INT, PRIMARY KEY (mid));
...
CREATE TABLE MovieDirector(mid INT, did INT, PRIMARY KEY (did));

CREATE TABLE MovieActor(mid INT, aid INT, role VARCHAR(50), PRIMARY KEY (aid));

CREATE TABLE MovieRating(mid INT, imdb INT, rot INT, PRIMARY KEY (mid));

CREATE TABLE Review(name VARCHAR(20), time TIMESTAMP, mid INT, rating INT, comment VARCHAR(500), PRIMARY KEY (name, time, mid));
